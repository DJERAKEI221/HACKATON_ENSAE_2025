...
dir.create("www/images/candidates", recursive = TRUE, showWarnings = FALSE)
...
# Création de la table voters si elle n'existe pas
dbExecute(con, "
...
if (all(c("positions", "candidates", "votes", "voters") %in% tables)) {
  ...
  message(sprintf("Nombre de candidats: %d", cand_count))
}
