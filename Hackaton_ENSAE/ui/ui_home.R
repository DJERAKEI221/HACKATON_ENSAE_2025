  ...
  fluidRow(class = "mb-5",
    ...
    column(6,
      div(class = "card info-card", style = "border-radius: 20px !important;",
        ...
        div(class = "card-body",
          p(class = "lead", "L'Amical des Elèves et Stagiaire de l'ENSAE (AES) est l'organisation représentative de tous les étudiants et stagiaires de l'ENSAE."),
          p("Fondée pour promouvoir la cohésion et défendre les intérêts des étudiants et stagiaires, l'AES organise de nombreuses activités académiques, culturelles et sportives tout au long de l'année."),
          p("Notre mission est de faire de votre parcours à l'ENSAE une expérience enrichissante tant sur le plan académique que social."),
          
          h4(class = "mt-4 mb-3", "Notre mission"),
          div(class = "mission-items",
            div(class = "d-flex align-items-center mb-3",
              div(style = "width: 30px; text-align: center; margin-right: 15px;",
                icon("check-circle", class = "text-primary")
              ),
              div("Représenter les étudiants auprès de l'administration")
            ),
            div(class = "d-flex align-items-center mb-3",
              div(style = "width: 30px; text-align: center; margin-right: 15px;",
                icon("check-circle", class = "text-primary")
              ),
              div("Organiser des événements culturels et sportifs")
            ),
            div(class = "d-flex align-items-center mb-3",
              div(style = "width: 30px; text-align: center; margin-right: 15px;",
                icon("check-circle", class = "text-primary")
              ),
              div("Faciliter l'intégration des nouveaux étudiants")
            ),
            div(class = "d-flex align-items-center mb-3",
              div(style = "width: 30px; text-align: center; margin-right: 15px;",
                icon("check-circle", class = "text-primary")
              ),
              div("Renforcer les liens avec les entreprises partenaires")
            ),
            div(class = "d-flex align-items-center mb-3",
              div(style = "width: 30px; text-align: center; margin-right: 15px;",
                icon("check-circle", class = "text-primary")
              ),
              div("Promouvoir l'excellence académique de l'ENSAE")
            )
          )
        )
      )
        ...
        div(class = "card-body",
          ...
          div(class = "document-item d-flex align-items-center",
            ...
            div(class = "flex-grow-1",
              ...
              p(class = "mb-0 small text-muted", "Document officiel - PDF (850 KB)")
            ),
            div(
              tags$a(href = "#", download = TRUE, class = "btn btn-sm btn-outline-primary", icon("download"))
            )
          ...
          div(class = "row align-items-center",
            div(class = "col-md-4",
              div(class = "contact-item mb-3",
                div(class = "icon-circle me-3", style = "background-color: rgba(63, 81, 181, 0.1); width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center;",
                  icon("globe", class = "text-primary")
                ),
                div(class = "flex-grow-1",
                  h5(class = "mb-1", "Site web"),
                  tags$a(href = "https://www.ensae.sn/", target = "_blank", class = "text-primary", "www.ensae.sn")
                )
              ),
              div(class = "contact-item mb-3",
                div(class = "icon-circle me-3", style = "background-color: rgba(63, 81, 181, 0.1); width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center;",
                  icon("envelope", class = "text-primary")
                ),
                div(class = "flex-grow-1",
                  h5(class = "mb-1", "Email"),
                  tags$a(href = "mailto:contact@ensae.sn", class = "text-primary", "contact@ensae.sn")
                )
              )
            ),
            div(class = "col-md-4",
              div(class = "contact-item mb-3", 
                div(class = "icon-circle me-3", style = "background-color: rgba(63, 81, 181, 0.1); width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center;",
                  icon("phone", class = "text-primary")
                ),
                div(class = "flex-grow-1",
                  h5(class = "mb-1", "Téléphone"),
                  p(class = "mb-0", "+221 77 028 69 51"),
                  p(class = "mb-0", "+221 78 109 18 22")
                )
              )
            ),
            div(class = "col-md-4 text-center",
              h5(class = "mb-3", "Suivez-nous"),
              div(class = "social-icons d-flex justify-content-center gap-4",
                tags$a(href = "https://www.facebook.com/AesEnsae", target = "_blank", class = "social-icon",
                       style = "width: 60px; height: 60px; display: flex; align-items: center; justify-content: center; border-radius: 15px; text-decoration: none; border: none;",
                       icon("facebook", class = "fa-2x text-primary")),
                tags$a(href = "https://www.linkedin.com/school/ensae", target = "_blank", class = "social-icon", 
                       style = "width: 60px; height: 60px; display: flex; align-items: center; justify-content: center; border-radius: 15px; text-decoration: none; border: none;",
                       icon("linkedin", class = "fa-2x text-primary")),
                tags$a(href = "https://www.instagram.com/aes.ensae?igsh=cWRsYWVmemJ5ZGE3", target = "_blank", class = "social-icon",
                       style = "width: 60px; height: 60px; display: flex; align-items: center; justify-content: center; border-radius: 15px; text-decoration: none; border: none;",
                       icon("instagram", class = "fa-2x text-primary"))
              )
            )
          )
        )
