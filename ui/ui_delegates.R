div(class = "container delegates-container",
  # CSS stylesheet
  tags$head(
    tags$link(rel = "stylesheet", type = "text/css", href = "css/delegates.css")
  ),
  ...
  tags$script(HTML("
    $(document).ready(function() {
      ...
      $('.delegate-card, .results-card').css({
        'width': '100%',
        'margin-bottom': '20px'
      });
      
      // Assurer que les conteneurs d'onglets sont correctement dimensionnés
      $('#delegate_type, #delegate_type > .tab-content').css('width', '100%');
      
      // Améliorer l'alignement de la grille de boutons
      ...
      $(window).resize(function() {
        $('.delegates-container, .delegate-card, .results-card, #delegate_type, .class-buttons-grid, .chart-container').css('width', '100%');
      });
    });
  ")),
  ...
  tags$style(HTML("
    ...
    /* Styles pour les options de candidats */
    .candidate-option {
      transition: all 0.3s ease;
      background-color: #f8f9fa;
  ...
  tags$script(HTML("
    $(document).ready(function() {
      ...
      // Animation pour les sections de candidats
      $('.candidates-section').hide().fadeIn(1000);
      
      // Gestion des radio buttons pour les délégués
      $(document).on('change', 'input[name=\"main_delegate_choice\"]', function() {
        var selectedValue = $(this).val();
        Shiny.setInputValue('main_delegate_choice', selectedValue);
        console.log('Délégué principal sélectionné:', selectedValue);
      });
      
      $(document).on('change', 'input[name=\"deputy_delegate_choice\"]', function() {
        var selectedValue = $(this).val();
        Shiny.setInputValue('deputy_delegate_choice', selectedValue);
        console.log('Délégué adjoint sélectionné:', selectedValue);
      });
      
      // Forcer l'affichage des sections de candidats
      $('.candidates-section, .main-delegates-section, .deputy-delegates-section').show();
  ...
  div(class = "delegate-card shadow-sm border-primary", style = "margin-bottom: 30px;",
    ...
    div(class = "card-body p-4",
      ...
      # Affichage automatique de la classe de l'étudiant authentifié
      div(class = "student-class-display mb-4", 
        uiOutput("selected_class_display")
      )
      ,
      
      # Affichage direct des candidats (sans onglets)
      div(class = "candidates-section mt-4",
        # Section délégués principaux
        div(class = "main-delegates-section mb-4",
          uiOutput("main_delegate_choices")
        ),
        
        # Section délégués adjoints
        div(class = "deputy-delegates-section mb-4",
  ...
  div(class = "results-card shadow-sm border-success mt-4",
    ...
    div(class = "card-body p-4",
      ...
      div(class = "results-class-selector mb-4 floating-element",
        ...
        # (SUPPRIMÉ : uiOutput("selected_results_class_display") et son conteneur)
      ), 
      hr(),
      hr(),
      hr(),
