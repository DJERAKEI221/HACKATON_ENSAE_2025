div(class = "container results-container",
  # CSS stylesheet
  tags$head(
    tags$link(rel = "stylesheet", type = "text/css", href = "css/results.css")
  ),
  ...
  tags$script(HTML("
    $(document).ready(function() {
      ...
      // Améliorer l'affichage de la carte principale des résultats
      $('.results-main-card').css({
        'width': '100%',
        'margin-bottom': '20px'
      });
      
      // Assurer que les rangées et colonnes sont correctement dimensionnées
      $('.results-filters').css('width', '100%');
      
      // Corriger l'affichage du conteneur de graphique
      $('.results-chart-container').css('width', '100%');
      
      // Appliquer les styles lors du redimensionnement de la fenêtre
      $(window).resize(function() {
        $('.results-container, .results-main-card, .results-filters, .results-chart-container').css('width', '100%');
      });
    });
  ")),
